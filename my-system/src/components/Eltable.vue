<template>
  <div class="elTableContainer">
    ...
    <div class="searchAndAdd">
      ...
      <el-button type="primary" v-if="tabName === 'second'" @click="add">添加</el-button>
    </div>
    <el-table
    ...
    >
      ...
      <el-table-column label="操作" v-if="tabName === 'second'">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="returnBook(scope.row)">还书</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
  export default {
    props: ['tableArr', 'getKeyword', 'keyword', 'tabName', 'getAddBtn', 'returnBookById'],
    data() {
      return {
        currentPage: 1,
        pageSize: 8,
